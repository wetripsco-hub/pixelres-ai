      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-32">
          {pricingTiers.map((tier, i) => (
            ...
            >
              <Card
                className={`bg-white/80 dark:bg-slate-900/50 border-slate-200 dark:border-white/10 flex flex-col justify-between h-full backdrop-blur-md transition-all hover:border-orange-500/50 dark:hover:border-cyan-500/50 rounded-[2.5rem] shadow-xl ${tier.tier === '4k' ? 'ring-2 ring-orange-500/50 dark:ring-cyan-500/50 shadow-orange-500/10 dark:shadow-[0_0_35px_rgba(6,182,212,0.15)] relative transform md:-translate-y-4' : ''}`}
              >
                {tier.tier === '4k' && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500 dark:from-cyan-500 dark:to-violet-600 text-white text-xs font-extrabold px-4 py-1 rounded-full uppercase tracking-wider shadow-lg">
                    Most Popular
                  </div>
                )}
                <CardHeader className="p-8">
                  <Badge variant="outline" className={`w-fit mb-6 text-xs font-bold uppercase ${tier.tier === '4k' ? 'border-orange-500/50 text-orange-700 dark:text-cyan-400 bg-orange-500/10 dark:bg-cyan-500/10' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800/50'}`}>
                    {tier.tier.toUpperCase()} Target
                  </Badge>
                  <CardTitle className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight">{tier.label}</CardTitle>
                  <div className="mt-6 flex items-baseline gap-1">
                    <span className={`text-5xl font-black font-mono ${tier.tier === '4k' ? 'text-orange-600 dark:text-cyan-400' : 'text-slate-900 dark:text-white'}`}>{tier.formattedPrice}</span>
                ...
                <CardContent className="px-8 space-y-3.5 text-sm text-slate-700 dark:text-slate-300">
                  <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-orange-500 dark:text-cyan-400" /> High Priority GPU Queue</div>
                  <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-orange-500 dark:text-cyan-400" /> Face & Texture Restoration</div>
                  <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-orange-500 dark:text-cyan-400" /> Direct High-Res Download</div>
                </CardContent>
                <CardFooter className="p-8 mt-4 border-t border-slate-100 dark:border-white/10">
                  <Link href={`/studio?tier=${tier.tier}`} className="w-full">
                    <Button
                      className={`w-full h-13 rounded-2xl text-base font-extrabold transition-all shadow-md btn-interactive ${tier.tier === '4k' ? 'bg-gradient-to-r from-orange-500 via-amber-500 to-orange-500 dark:from-cyan-600 dark:to-cyan-500 hover:opacity-95 text-white shadow-orange-500/20 dark:shadow-cyan-500/25' : 'bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white'}`}
                    >
                      Start Enhancing
                    </Button>
                  </Link>
